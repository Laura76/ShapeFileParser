...
public class SHP_Header {
  ...
  private File file;
  private final static int SHP_MAGIC   = 9994;//�ļ�����
  private final static int SHP_VERSION = 1000;//�汾
  
  private int SHP_file_length, SHP_shape_type;//�ļ�����-�ļ�����
  ...
  public void read(ByteBuffer bb) throws Exception{
    // MAIN FILE HEADER
    bb.order(ByteOrder.BIG_ENDIAN);//���ֽ�
    // magic number
    int SHP_MAGIC_read   = bb.getInt( 0);//�ļ�ͷ��ħ�����ļ�����9994
    if( SHP_MAGIC_read != SHP_MAGIC ){
      throw new Exception("(ShapeFile) error: SHP_MAGIC = "+SHP_MAGIC+", File: "+file );
    }
    // file length
    SHP_file_length  = bb.getInt(24);//�ļ�����
    bb.order(ByteOrder.LITTLE_ENDIAN);//С�ֽ�
    int SHP_version_read      = bb.getInt(28);//�ļ��汾1000
    if( SHP_version_read != SHP_VERSION ){
      throw new Exception("(ShapeFile) error: SHP_VERSION = "+SHP_VERSION+", File: "+file );
    }
    
    SHP_shape_type = bb.getInt(32);//shape����
   
    try {
      shape_type = ShpShape.Type.byID(SHP_shape_type);//����ID��ȡshapetype
    } catch (Exception e) {
      e.printStackTrace();
    }
